...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []ufoutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []ufoutil.Address, int, error) {
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []ufoutil.Address,
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []ufoutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*ufoutil.AddressPubKey)
...
type KeyDB interface {
	GetKey(ufoutil.Address) (*btcec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(ufoutil.Address) (*btcec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address ufoutil.Address) (*btcec.PrivateKey,
	bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(ufoutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(ufoutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address ufoutil.Address) ([]byte, error) {
	return sc(address)
}
