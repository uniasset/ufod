...
// newAddressPubKey returns a new ufoutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) ufoutil.Address {
	addr, err := ufoutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new ufoutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) ufoutil.Address {
	addr, err := ufoutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new ufoutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) ufoutil.Address {
	addr, err := ufoutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the ufoutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the ufoutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := ufoutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := ufoutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := ufoutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := ufoutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := ufoutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*ufoutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*ufoutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*ufoutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*ufoutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*ufoutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
